    ...
    <div className="h-full flex flex-col bg-slate-900 rounded-xl border border-slate-700 shadow-xl overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                ...
                {currentStep?.event && currentStep.event !== 'line' && (
                  <span className={`text-xs px-2 py-0.5 rounded-full animate-scale-in ${
                    currentStep.event === 'call' ? 'bg-blue-500/20 text-blue-400' :
                    ...
                    return (
                      <div 
                        ...
                        className={`flex items-center justify-between p-3 rounded-lg transition-all duration-500 ${
                          isChanged ? 'bg-teal-500/15 border border-teal-500/30 animate-var-flash' : 'bg-slate-900/50'
                        }`}
                      >
                        <div className="flex items-center gap-3">
      ...
      {hasSteps && (
        <div className="px-4 py-4 bg-slate-800/30 border-t border-slate-700">
              ...
              {steps.length <= 20 && steps.map((_, idx) => (
                <div
                  ...
                  className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                    idx <= currentStepIndex ? 'bg-teal-400' : 'bg-slate-600'
          ...
          {/* Auto-play indicator */}
          {isAutoPlaying && (
            <div className="mt-3 flex items-center justify-center gap-2 text-teal-400 text-sm animate-pulse">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <polygon points="5,3 19,12 5,21" />
              </svg>
              <span>Auto-playing... (2s per step)</span>
            </div>
          )}
